...
import { saveBest, saveHintEnabled, saveSoundEnabled } from './settings.js';
...
export function resumeRun(){
  ...
  if(state.hintEnabled) dom.verseStrip.classList.remove('hidden');
  state.paused = false;
  startLoop();
}
...
export function beginLevel(){
  ...
  state.particles = []; state.decoys = []; state.activeTile = null;
  ...
  state.fallSpeed = p.fallSpeed; state.decoyInterval = p.decoyInterval; state.spawnDelay = p.spawnGap;
  ...
  dom.levelCount.textContent = state.words.length + ' words · find them in order';
  dom.verseStrip.classList.add('hidden');
  ...
  const iv = setInterval(()=>{
    ...
    if(c <= 0){
      ...
      dom.levelScreen.classList.add('hidden');
      if(state.hintEnabled) dom.verseStrip.classList.remove('hidden');
      else dom.verseStrip.classList.add('hidden');
      updateStrip();
      dom.pauseBtn.classList.add('show');
      state.phase = 'battle';
      state.spawnDelay = 0.4;
      startLoop();
    } else {
      dom.countdownVal.textContent = c;
    }
...
export function hideLeaderboard(){
  ...
  if(state.hasActiveRun || state.phase === 'gameover'){
